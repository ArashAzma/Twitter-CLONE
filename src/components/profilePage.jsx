  ...
  return (
    <div className="flex flex-col lg:flex-row-reverse  lg:items-start px-[75px] items-center justify-end gap-y-8 relative">
      <div className="flex flex-col gap-4 md:mx-[75px] max-h-[82vh] items-center mx-6 relative">
        <div className="absolute grid grid-cols-3 gap-4 divide-x divide-gray-400 items-center bg-[#1b2730] py-2 rounded-xl w-[300px] md:w-[500px]">
          <button
            ...
            className="focus:opacity-100 focus:text-[#1da1f2] opacity-40 duration-200 "
          >
            tweets
          </button>
          <button
          ...
          <button
            onClick={() => {
              setData(bookedTweets);
              setDataType("bookmarks");
            }}
            className="focus:opacity-100 focus:text-[#1da1f2] opacity-40 duration-200 "
          >
            bookmarks
              ...
              <div>
                {dataType == "tweets" ? (
                  <div className="flex flex-col justify-center items-center">
                    ...
                    <Link
                      href="/create_tweet"
                      className="hover:scale-105 smooth duration-150 text-[#1da1f2] underline font-bold"
                    >
      ...
      <div className="flex flex-col gap-4 mb-6">
            ...
            <div className="flex flex-col items-center p-6 relative">
              {
                <Image
                  ...
                  width={75}
                  height={75}
                  className="rounded-xl absolute -top-4"
                  alt="user"
                />
              }
              <div className="absolute top-24 ">
                ...
                <div className="flex gap-x-2 mb-5 justify-between items-center">
                  ...
                  <div className="flex gap-2">
                    {user.bio ? (
                      ...
                      <div className="opacity-70 font-light text-xs flex items-center gap-x-3">
                        Bio is Empty !
                      </div>
                    )}
                    {user._id == session?.user.id && (
                      <RiBallPenFill
                        ...
                        className="text-[17px] text-[#1da1f2] hover:text-sky-600 cursor-pointer "
                      />
                    )}
                  </div>
                </div>
                <div className="grid grid-cols-2 divide-x divide-gray-400  gap-x-2 mb-3 justify-between items-center">
                  <div
                    ...
                    className="flex flex-col items-center justify-center opacity-50 hover:opacity-90 cursor-pointer"
                  >
                    <div>Following :</div>
        ...
        {user?._id != session?.user.id && session && (
          <div>
            {!isFollowed ? (
              <button
                onClick={handleFollow}
                className=" w-[300px] bg-sky-500 px-2 py-1 rounded-lg"
              >
                Follow
              </button>
            ) : (
              <button
                ...
                className=" w-[300px] bg-gray-500 px-2 py-1 rounded-lg"
              >
                Unfollow
              </button>
            )}
      ...
      {bioOpen && (
        <>
          ...
          <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2  w-[350px] md:w-[400px] h-[200px] bg-[#06141d] rounded-xl flex items-center justify-center">
            <button
              onClick={handleBioClose}
              className="absolute right-5 top-4 hover:text-red-800 duration-200 text-lg"
            >
              <RiCloseFill />
            </button>
            <form
            ...
            >
              ...
              <button
                type="submit"
                className="bg-green-700 rounded-lg w-[150px] opacity-70 hover:opacity-90 duration-200"
              >
                submit
              </button>
            </form>
